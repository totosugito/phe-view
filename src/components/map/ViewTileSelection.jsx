    ...
    <ViewInfoLayout title={t("map.mapLayers")} visible={visible} setVisible={setVisible}>
      {mapTileList.map((layer) => (
        <div key={layer.key} onClick={() => handleLayerChange(layer)}
          ...
          <div style={{ position: "absolute" }} className={"p-1 bg-base-300 rounded-br-md rounded-tl-md"}>
            <div className={"text-sm text-black opacity-100 font-semibold"}>
              {layer.name}
            </div>
          </div>
          <img src={layer.thumbnail} alt={layer.name} className={"h-[90px] w-full object-cover"} />
        </div>
      ))}
    </ViewInfoLayout>
  );
